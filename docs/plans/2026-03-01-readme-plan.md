...
**Goal:** Write a comprehensive `README.md` at the project root covering setup, usage (watcher + CLI), troubleshooting, and development reference.

**Architecture:** Single markdown file at the project root. No code changes — documentation only. All content approved in the design doc at `docs/plans/2026-03-01-readme-design.md`.

**Tech Stack:** Markdown, GitHub-flavored (GFM). No tooling needed — just write the file.

---

## Reference Material

Before writing, check these for accurate details:
- `.env.example` — exact env var names and defaults
- `src/mcp/gmail_client.py:30-51` — `AI_LABELS` list (the label tree)
- `src/mcp/gmail_client.py:433-458` — MCP server startup env vars (WORKSPACE_MCP_PORT etc.)
- `.context/ARCHITECTURE.md` — architecture overview
- `.context/CURRENT_STATUS.md` — current CLI commands and test counts

---

### Task 1: Write Header, Overview, and Prerequisites

**Files:**
- Create: `README.md`

**Step 1: Write the file with the first two sections**
...
**Step 2: Verify the file renders — scan for obvious formatting issues**

Open `README.md` and read through it. Check that the ASCII diagram aligns
correctly (monospace only — no smart quotes or non-ASCII characters).

**Step 3: Commit**

```bash
git add README.md
git commit -m "docs(readme): header, overview, prerequisites"
```

---

### Task 2: Step 1 — Anthropic API Key

**Files:**
- Modify: `README.md` (append)

**Step 1: Append the Anthropic key section**
...
```bash
git add README.md
git commit -m "docs(readme): Step 1 - Anthropic API key"
```

---

### Task 3: Step 2 — Google OAuth (the long section)

**Files:**
- Modify: `README.md` (append)

**Step 1: Append the full Google OAuth section**

Write the section exactly as designed. Key details to get right:
- Project dropdown is at the **top** of console.cloud.google.com
- OAuth consent screen: must select **External**, must **Publish App** (otherwise token expires in 7 days)
- Credentials type: **Desktop app** (not Web application)
- Client ID ends in `.apps.googleusercontent.com`
- `workspace-mcp` OAuth flow opens a browser — warn about "unverified app" prompt
...
```bash
git add README.md
git commit -m "docs(readme): Step 2 - Google OAuth walkthrough"
```

---

### Task 4: Step 3 — Install and Configure

**Files:**
- Modify: `README.md` (append)

**Step 1: Append the install and configure section**

Copy the exact env var names from `.env.example`. Annotate each one with
what it's for and where the value comes from. Note which vars can be left
at their defaults.
...
```bash
git add README.md
git commit -m "docs(readme): Step 3 - install and configure"
```

---

### Task 5: Step 4 — First Run and Verify

**Files:**
- Modify: `README.md` (append)

**Step 1: Append the first run section**
...
```bash
git add README.md
git commit -m "docs(readme): Step 4 - first run and verify"
```

---

### Task 6: Watcher Section

**Files:**
- Modify: `README.md` (append)

**Step 1: Append the watcher section**

Get the exact label tree from `src/mcp/gmail_client.py:30-51` (the
`AI_LABELS` list). Render it as a tree.
...
```bash
git add README.md
git commit -m "docs(readme): watcher section"
```
...
```bash
git add README.md
git commit -m "docs(readme): CLI section"
```
...
Key details to get right from the code:
- Retry count: 5 attempts, 3-second delay (from `gmail_client.py:397-398`)
- Port env var: `WORKSPACE_MCP_PORT` (from `gmail_client.py:441`)
- Token cache location: `~/.workspace-mcp/` (check if this is accurate or leave as "locally cached")
...
```bash
git add README.md
git commit -m "docs(readme): troubleshooting section"
```

---

### Task 9: Development Section

**Files:**
- Modify: `README.md` (append)

**Step 1: Append the development section**
...
**Step 2: Do a final read-through of the complete README**

Read `README.md` from top to bottom. Check:
- All code blocks are closed (every ` ``` ` has a matching close)
- No `[PLACEHOLDER]` text left in
- The label tree matches `src/mcp/gmail_client.py:30-51`
- All `uv run email-agent` commands match actual CLI commands

**Step 3: Final commit**

```bash
git add README.md
git commit -m "docs(readme): development section + final review"
```
